...
public class RegisterController extends JSFController {
	...
	public void checkAdminCreated()
			throws NoSuchAlgorithmException, UnsupportedEncodingException, EncodeFailedException {
	...
	public String register() {
		...
		if (confirmPassword.matches(".*\\s+.*") || user.getPassword().matches(".*\\s+.*")) {
			...
			context.addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, bundle.getString("signupForm.passwordBlank"), ""));
			return null;
		}
		...
		if (!user.getEmail().matches("^[A-Z0-9a-z._%+-]+@[a-zA-Z0-9.-]+\\.[a-zA-Z]{2,6}$")) {
			...
			ResourceBundle bundle = context.getApplication().getResourceBundle(context, "msgsCore");
			context.addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, bundle.getString("signupForm.validEmail"), ""));
			return null;
		}
		...
		if (confirmPassword.equals(user.getPassword())) {
			try {
				...
				ResourceBundle bundle = context.getApplication().getResourceBundle(context, "msgsCore");
				context.addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR, bundle.getString("signupForm.emailError"), ""));
				return null;
			} catch (EncodeFailedException e) {
				FacesContext context = FacesContext.getCurrentInstance();
				ResourceBundle bundle = context.getApplication().getResourceBundle(context, "msgsCore");
				context.addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR, bundle.getString("signupForm.encodeError"), ""));
				return null;
			}
			...
			ResourceBundle bundle = context.getApplication().getResourceBundle(context, "msgsCore");
			context.addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, bundle.getString("signupForm.passwordMismatch"), ""));
			return null;
		}
	...
	// Irá criar um objeto endereço que deverá ser preenchido na página
	// correspondente
	...
	// Função de atualiação de senha. Será chamada no formulário responsável por
	// essa funcionalidade.
	public String updatePassword()
			throws NoSuchAlgorithmException, UnsupportedEncodingException, EncodeFailedException {
		try {
			if (this.authenticateController.getUser().getPassword()
					.compareTo(TextUtils.produceBase64EncodedMd5Hash(this.oldPassword)) == 0) {
				if (this.newPassword.compareTo(this.confirmPassword) == 0) {
					this.authenticateController.getUser()
							.setPassword(TextUtils.produceBase64EncodedMd5Hash(this.newPassword));
					return this.updateUser();
				} else {
					FacesContext context = FacesContext.getCurrentInstance();
					ResourceBundle bundle = context.getApplication().getResourceBundle(context, "msgsCore");
					context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
							bundle.getString("user.updatePassPage.mismatch"), ""));
					return null;
				}
			} else {
				FacesContext context = FacesContext.getCurrentInstance();
				ResourceBundle bundle = context.getApplication().getResourceBundle(context, "msgsCore");
				context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						bundle.getString("user.updatePassPage.error"), ""));
				return null;
			}
	...
	// Função que cancela a mudança de senha do usuário. Será chamada no formulário
