package br.ufes.informatica.doeLivros.core.domain;

import java.util.Set;

import javax.persistence.Entity;
import javax.persistence.ManyToMany;
import javax.validation.constraints.NotNull;
import javax.validation.constraints.Size;

import br.ufes.inf.nemo.jbutler.ejb.persistence.PersistentObjectSupport;

/** TODO: generated by FrameWeb. Should be documented. */
@Entity
public class Role extends PersistentObjectSupport implements Comparable<Role> {
	/** Serialization id. */
	private static final long serialVersionUID = 1L;



	/** TODO: generated by FrameWeb. Should be documented. false */
	@NotNull @Size(max = 200) 
	private String name;




		
		/** TODO: generated by FrameWeb. Should be documented. */
		@ManyToMany(mappedBy="roleTarget")
		private Set<User> userSource;
		
	

		
		/** TODO: generated by FrameWeb. Should be documented. */
		@ManyToMany
		private Set<Permission> Target;
		
	




	/** Getter for name. */
	public String getName() {
		return name;
	}
	
	/** Setter for name. */
	public void setName(String name) {
		this.name = name;
	}




		
		/** Getter for Source. */
		public Set<User> getSource() {
			return userSource;
		}
		
		/** Setter for Source. */
		public void setSource(Set<User> Source) {
			this.userSource = Source;
		}
		
	

		
		/** Getter for Target. */
		public Set<Permission> getTarget() {
			return Target;
		}
		
		/** Setter for Target. */
		public void setTarget(Set<Permission> Target) {
			this.Target = Target;
		}
		
	





	/** @see java.lang.Comparable#compareTo(java.lang.Object) */
	@Override
	public int compareTo(Role o) {
		// FIXME: auto-generated method stub		
		return super.compareTo(o);
	}
}